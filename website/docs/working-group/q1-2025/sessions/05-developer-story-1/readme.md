# Session 05: Developer Story #1


This session features real stories and experiences from developers building on Cardano.

### Session Topics
- Developer journey showcase
- Challenges and solutions
- Lessons learned from building on Cardano
- Community insights and advice

**Status**: Session materials in development

---

*This session is part of the Q1 2025 Developer Experience Working Group: "Laying the Foundations"*# Session 05: Developer Story #1 – Community Builders Edition

This session features real stories and lived experiences from community builders actively growing the Cardano developer ecosystem across different regions.

## Featured Sessions

### 🎥 Darlington Wleh — Community Building from the Ground Up
In this session, Darlington Wleh shares his journey as a grassroots community builder within the Cardano ecosystem. He walks through how local developer communities are formed, the early challenges of onboarding new builders, and the importance of consistency, trust, and mentorship. The session highlights practical lessons on sustaining engagement, nurturing first-time contributors, and creating safe spaces for developers to learn and collaborate.

<iframe
  src="https://www.youtube.com/embed/JQ_TQQ4N7TY"
  title="Session 05: Developer Story - Darlington"
  width="100%"
  height="480"
  allow="autoplay"
  allowfullscreen
  style={{border: 0, borderRadius: '12px', boxShadow: '0 16px 40px rgba(1, 40, 170, 0.18)'}}
/>

---

### 🎥 Dan Barika — Bridging Developers, Education, and Ecosystem Growth
Dan Barika discusses his experience building developer communities through education, workshops, and ecosystem alignment. He explores how community builders can act as bridges between developers, projects, and working groups, while sharing insights on identifying developer pain points and turning them into actionable programs. The session emphasizes long-term ecosystem thinking and the role of community leadership in scaling Cardano adoption.


<iframe
  src="https://www.youtube.com/embed/qXEO-bZ-tAk"
  title="Session 05: Developer Story - Dan Baruka"
  width="100%"
  height="480"
  allow="autoplay"
  allowfullscreen
  style={{border: 0, borderRadius: '12px', boxShadow: '0 16px 40px rgba(1, 40, 170, 0.18)'}}
/>

---

## Key Themes Across Both Sessions
- Real-world community building experiences  
- Challenges in onboarding and retaining developers  
- Lessons learned from ecosystem growth efforts  
- Advice for new and aspiring community builders  
- The role of working groups in developer success  

**Status**: Session videos to be embedded

---

*This session is part of the Q1 2025 Developer Experience Working Group: **“Laying the Foundations”***