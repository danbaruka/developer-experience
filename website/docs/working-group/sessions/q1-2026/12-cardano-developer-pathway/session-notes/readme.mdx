...
The **diagram below** is one visual overview of typical journeys on Cardano from where you start, through learning and practice, into tracks (contracts, frontend, infra), integration, testing, testnets, and deeper specialisations. It is **illustrative only**: we have **not** tried to list every tool, course, or link; many things are left out so the chart stays usable. **Click** a node to open a related page where we attached one; **hover** may show a short tooltip from the diagram renderer. Use the toolbar for **full view** and **zoom** (or **Ctrl/Cmd + scroll** over the diagram).
